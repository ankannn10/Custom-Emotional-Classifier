...
def preprocess_text(text):
    ...
    # Step 4: Remove Unwanted Characters (Keep emoji tokens intact)
...
def clean_comment(comment):
    """
    Cleans a single comment by removing URLs, excessive whitespace, emojis, slang, and special characters.
...
def clean_transcript(transcript):
    """
    Cleans transcript text by removing unwanted elements and formatting.
    """
    if not transcript:
        return ""
...
def chunk_text(text, max_length=128, overlap=32):
    """
    Splits the given text into chunks of a specified max length with overlap.
    """
    words = text.split()
    chunks = []
...
def process_cleaning(input_file, output_file, max_chunk_length=128, chunk_overlap=32):
    """
    Cleans transcript and comments from a JSON file and pairs them into a CSV.
    
    Args:
        input_file (str): Path to input JSON file.
        output_file (str): Path to output CSV file.
        max_chunk_length (int): Max length of each transcript chunk.
    ...
    try:
        ...
        chunks_df.to_csv('output2/chunks.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--chunk_length", type=int, default=128, help="Maximum length of each transcript chunk.")
    parser.add_argument("--chunk_overlap", type=int, default=32, help="Overlap between transcript chunks.")
