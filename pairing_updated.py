...
def validate_csv(input_file):
    """
    Validate if the input CSV contains the required columns.
    """
...
def load_data(pairs_csv):
    """
    Load transcript-comment pairs from a CSV file.
    """
    pairs = []
...
def find_most_relevant_chunk(pairs_csv, output_csv):
    """
    Finds the most relevant transcript chunk for each comment using Sentence-BERT embeddings and cosine similarity.
    """
    device = get_device()
